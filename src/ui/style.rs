...
pub fn truncate(s: &str, max: usize) -> String {
    ...
    if max == 0 {
        // Ноль ячеек — это ноль ячеек: многоточие тут само по себе шире
        // отведённого, и строка вылезала за край ровно на него.
        return String::new();
    }
    if max == 1 {
        return "…".into();
...
mod tests {
    ...
    fn truncation_counts_letters_and_carries_the_colour() {
        ...
        // В ноль ячеек не влезает и многоточие.
        assert_eq!(truncate("абв", 0), "");
        assert_eq!(truncate("абв", 1), "…");
    }
