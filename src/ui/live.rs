...
pub fn map_key(view_is_text: bool, k: KeyEvent) -> Act {
    ...
    if ctrl {
        return match k.code {
            ...
            // Ctrl+L — перерисовать начисто. Соглашение из шелла: экран
            // портит всё, что печатает мимо нас (чужая программа, вывод ssh),
            // и человеку нужен способ сказать «нарисуй заново».
            KeyCode::Char('l') => Act::Redraw,
...
impl RawGuard {
    fn enable() -> Self {
        ...
        // различать Shift+Enter и Enter. Просим ровно «различать коды» —
        // сообщать об отпускании не просим, иначе каждое нажатие приезжало бы
        // дважды.
        //
        // Спрашивать разрешения нельзя: `supports_keyboard_enhancement()` ждёт
        // ответа терминала до двух секунд, и на всех, кто протокол не знает,
        // окно ровно столько и не появлялось. Измерено: 0,28 с против 2,24 с.
        // Поэтому просто просим — незнакомую последовательность терминал по
        // стандарту пропускает, а кому она всё же навредит, тот выключит
        // переменной JARVIS_NO_KITTY.
        let kitty = raw && std::env::var_os("JARVIS_NO_KITTY").is_none();
        if kitty {
