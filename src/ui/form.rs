...
impl Form {
    ...
    /// Вернуться к прошлому вопросу и отдать то, что было отвечено.
    ///
    /// `None` — возвращаться некуда. Форма без «назад» заставляет отменять всё
    /// и набирать заново из-за одной опечатки в первом же ответе.
    pub fn back(&mut self) -> Option<String> {
        ...
        Some(match self.step() {
            Step::Dir => self.dir.clone(),
            Step::Name => self.name.clone(),
            Step::Base => self.base.clone(),
            Step::Goal => self.goal.clone(),
            // Шаги с выбором номеров переспрашиваем начисто: подставлять в
            // строку «1,3» нечего — там был выбор из списка.
            Step::Gates | Step::Source => String::new(),
        })
    }
